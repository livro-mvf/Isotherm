...
//               equações da isoterma de Henderson.
//
...
#include <Isotherm/ThreeParameters/Henderson.h>
...
//==============================================================================
    ...
    PrintLine(std::cout);    
    std::cout << "Teste da classe Henderson\n";
    PrintLine(std::cout);    
...
ist::Henderson     f1;                 //Construtora default    
    assert (f1.K1() == 0); 
    assert (f1.K2() == 0); 
    assert (f1.Qmax() == 0); 
...
ist::Henderson     f2( QMAX, K1, K2 );     // Construtora com 3 parâmetros; 
    assert (f2.K1() == K1); 
    assert (f2.K2() == K2); 
    assert (f2.Qmax() == QMAX);
...
ist::Henderson     f3(f2);             // Construtoa de cópia
    assert (f2.K1() == f3.K1()); 
    assert (f2.K2() == f3.K2()); 
...
ist::Henderson     f4 = ist::Henderson(QMAX1, K11, K22);
    assert (f4.K1() == K11); 
    assert (f4.K2() == K22); 
...
const ist::Henderson            var_H(QMAX_QE, K1_QE, K2_QE);
const Real                      QEANALITICO(0.258353824);
const Real                      QeNumerico(var_H.Qe(CE));
...
ist::Henderson     f6 ( - QMAX_Errado,    K1_Errado,    K2_Errado);              

ist::Henderson     f7 (   QMAX_Errado,  - K1_Errado,    K2_Errado);              

ist::Henderson     f8 (   QMAX_Errado,    K1_Errado,  - K2_Errado);             
