//==============================================================================
// Name        : TesteBrouersSotolongo.cpp
...
//               equações da isoterma de Brouers - Sotolongo.
//
...
#include <Isotherm/ThreeParameters/BrouersSotolongo.h>      // ist::BrouersSotolongo
...
//==============================================================================
    ...
    PrintLine(std::cout);    
    std::cout << "Teste da classe Brouers - Sotolongo\n";
    PrintLine(std::cout);    
...
ist::BrouersSotolongo     f1;                 //Construtora default    
    assert (f1.K1() == 0); 
    assert (f1.K2() == 0); 
    assert (f1.Qmax() == 0); 
...
ist::BrouersSotolongo     f2( QMAX, K1, K2 );     // Construtora com 3 parâmetros; 
    assert (f2.K1() == K1); 
    assert (f2.K2() == K2); 
    assert (f2.Qmax() == QMAX);
...
ist::BrouersSotolongo     f3(f2);             // Construtoa de cópia
    assert (f2.K1() == f3.K1()); 
    assert (f2.K2() == f3.K2()); 
...
ist::BrouersSotolongo     f4 = ist::BrouersSotolongo(QMAX1, K11, K22);
    assert (f4.K1() == K11); 
    assert (f4.K2() == K22); 
...
const Real                      CE(0.1);
const Real                      K1_QE(4.32e-1);
const Real                      K2_QE(2.372e-1);
const Real                      QMAX_QE(0.6);
const ist::BrouersSotolongo     var_BS(QMAX_QE, K1_QE, K2_QE);
const Real                      QEANALITICO(0.1328120191);
const Real                      QeNumerico(var_BS.Qe(CE));
...
Real dQAnalitico = 0.2772618348;  
Real Errodqdc = fabs(var_BS.DQDC(CE)- dQAnalitico) / dQAnalitico;  
...
ist::BrouersSotolongo     f6  ( - QMAX_Errado,    K1_Errado,    K2_Errado);            // Teste ok   
ist::BrouersSotolongo     f61 (             0,    K1_Errado,    K2_Errado);            // Teste ok   

ist::BrouersSotolongo     f7  (   QMAX_Errado,  - K1_Errado,    K2_Errado);            // Teste ok   
ist::BrouersSotolongo     f71 (   QMAX_Errado,            0,    K2_Errado);            // Teste ok   

ist::BrouersSotolongo     f8  (   QMAX_Errado,    K1_Errado,  - K2_Errado);            // Teste ok   
ist::BrouersSotolongo     f81 (   QMAX_Errado,    K1_Errado,            0);            // Teste ok   

    std::cout   << "Imprimindo Qe com C errada = " 
                << f1.Qe( - CE_Errado ) 
                << std::endl;                                // Teste ok
    
ist::BrouersSotolongo     f9;    
