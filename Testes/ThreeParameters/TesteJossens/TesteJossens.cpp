...
// Version     : 1.0
// Description : Programa de teste da classe Jossens da biblioteca isothermlib
//
...
int main(int argc, char** argv) {
    ...
    PrintLine(std::cout);    
    std::cout << "Teste da classe Jossens\n";
    PrintLine(std::cout);    
...
ist::Jossens     f1;                 //Construtora default    
    assert (f1.K1() == 0); 
    assert (f1.K2() == 0); 
    assert (f1.Qmax() == 0);
...
ist::Jossens   f2(QMAX, K1, K2);       // Construtora com 3 parametros
    assert (f2.K1() == K1); 
    assert (f2.K2() == K2); 
    assert (f2.Qmax() == QMAX);
...
ist::Jossens   f3(f2);             // Construtora de cópia
    assert (f2.K1() == f3.K1()); 
    assert (f2.K2() == f3.K2()); 
...
ist::Jossens  f4 = ist::Jossens(QMAX1, K11, K22);
    assert (f4.K1() == K11); 
    assert (f4.K2() == K22); 
...
const ist::Jossens              var_SP(QMAX_QE, K1_QE, K2_QE);
...
ist::Jossens     f6 ( - QMAX_Errado,    K1_Errado,    K2_Errado);             
ist::Jossens     f61(             0,    K1_Errado,    K2_Errado);              

ist::Jossens     f7 (   QMAX_Errado,  - K1_Errado,    K2_Errado);             
ist::Jossens     f71(   QMAX_Errado,            0,    K2_Errado);            

ist::Jossens     f8 (   QMAX_Errado,    K1_Errado,  - K2_Errado);             
ist::Jossens     f81(   QMAX_Errado,    K1_Errado,            1);              
