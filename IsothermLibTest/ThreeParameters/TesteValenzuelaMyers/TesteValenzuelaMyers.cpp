//==============================================================================
// Name        : TesteValenzuelaMyers.cpp
// Authors     : Lara Botelho Brum
//               Luan Rodrigues Soares de Souza
//               Joao Flavio Vieira de Vasconcellos
// Version     : 1.0
// Description : Programa para testar a classe com as 
//               equações da isoterma de Valenzuela - Myers.
//
...
//==============================================================================
    ...
    PrintLine(std::cout);    
    std::cout << "Teste da classe Valenzuela - Myers\n";
    PrintLine(std::cout);    
...
ist::ValenzuelaMyers     f1;                 //Construtora default    
    assert (f1.K1() == 0); 
    assert (f1.K2() == 0); 
    assert (f1.Qmax() == 0); 
...
ist::ValenzuelaMyers     f2( QMAX, K1, K2 );     // Construtora com 3 parâmetros; 
    assert (f2.K1() == K1); 
    assert (f2.K2() == K2); 
    assert (f2.Qmax() == QMAX);
...
ist::ValenzuelaMyers     f3(f2);             // Construtoa de cópia
    assert (f2.K1() == f3.K1()); 
    assert (f2.K2() == f3.K2()); 
...
ist::ValenzuelaMyers     f4 = ist::ValenzuelaMyers(QMAX1, K11, K22);
    assert (f4.K1() == K11); 
    assert (f4.K2() == K22); 
...
const ist::ValenzuelaMyers           var_VM(QMAX_QE, K1_QE, K2_QE);
const Real                      QEANALITICO(0.258353824);
const Real                      QeNumerico(var_VM.Qe(CE));
...
ist::ValenzuelaMyers     f6 ( - QMAX_Errado,    K1_Errado,    K2_Errado);            // Teste ok   

ist::ValenzuelaMyers     f7 (   QMAX_Errado,  - K1_Errado,    K2_Errado);            // Teste ok   

ist::ValenzuelaMyers     f8 (   QMAX_Errado,    K1_Errado,  - K2_Errado);            // Teste ok   

    std::cout   << "Imprimindo Qe com C errada = " 
                << f1.Qe( - CE_Errado ) 
                << std::endl;                                // Teste ok
    
ist::ValenzuelaMyers     f9;    
