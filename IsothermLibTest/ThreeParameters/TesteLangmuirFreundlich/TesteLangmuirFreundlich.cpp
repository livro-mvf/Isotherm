...
//               equações da isoterma de Langmuir - Freundlich.
//
...
//==============================================================================
    ...
    PrintLine(std::cout);    
    std::cout << "Teste da classe Langmuir - Freundlich\n";
    PrintLine(std::cout);    
...
ist::LangmuirFreundlich     f1;                 //Construtora default    
    assert (f1.K1() == 0); 
    assert (f1.K2() == 0); 
    assert (f1.Qmax() == 0); 
...
ist::LangmuirFreundlich     f2( QMAX, K1, K2 );     // Construtora com 3 parâmetros; 
    assert (f2.K1() == K1); 
    assert (f2.K2() == K2); 
    assert (f2.Qmax() == QMAX);
...
ist::LangmuirFreundlich     f3(f2);             // Construtoa de cópia
    assert (f2.K1() == f3.K1()); 
    assert (f2.K2() == f3.K2()); 
...
ist::LangmuirFreundlich     f4 = ist::LangmuirFreundlich(QMAX1, K11, K22);
    assert (f4.K1() == K11); 
    assert (f4.K2() == K22); 
...
const ist::LangmuirFreundlich           var_FS(QMAX_QE, K1_QE, K2_QE);
const Real                      QEANALITICO(0.301417972);
const Real                      QeNumerico(var_FS.Qe(CE));
...
ist::LangmuirFreundlich     f6 ( - QMAX_Errado,    K1_Errado,    K2_Errado);            // Teste ok   

ist::LangmuirFreundlich     f7 (   QMAX_Errado,  - K1_Errado,    K2_Errado);            // Teste ok   

ist::LangmuirFreundlich     f8 (   QMAX_Errado,    K1_Errado,  - K2_Errado);            // Teste ok   

    std::cout   << "Imprimindo Qe com C errada = " 
                << f1.Qe( - CE_Errado ) 
                << std::endl;                                // Teste ok
    
ist::LangmuirFreundlich     f9;    
