...
// Version     : 1.0
// Description : Programa de teste da classe Sips
//
...
const Real              QMAX(4.548);    
const Real              K1(1e-02); 
const Real              K2(1e-03);

ist::Sips     f1;                 //Construtora default    
    assert (f1.K1() == 0); 
    assert (f1.K2() == 0); 
    assert (f1.Qmax() == 0);
...
ist::Sips    f2( QMAX, K1, K2 );     // Construtora com 2 parâmetros;                     //Construtora default    
    assert (f2.K1() == K1); 
    assert (f2.K2() == K2); 
    assert (f2.Qmax() == QMAX);
...
ist::Sips     f3(f2);             // Construtoa de cópia
    assert (f2.K1() == f3.K1()); 
    assert (f2.K2() == f3.K2()); 
...
const Real QMAX1 (100.0);
const Real K11 (1.0);
const Real K22 (2.0);

ist::Sips     f4 = ist::Sips(QMAX1, K11, K22);  
    assert (f4.K1() == K11); 
    assert (f4.K2() == K22); 
...
const ist::Sips               var_SP(QMAX_QE, K1_QE, K2_QE);
const Real                      QeNumer(var_SP.Qe(CE));
const Real                      QEANALIT(0.47527961);
...
ist::Sips     f6 ( - QMAX_Errado,    K1_Errado,    K2_Errado);            // Teste ok   
ist::Sips     f61(           0,    K1_Errado,    K2_Errado);            // Teste ok  

ist::Sips     f7 (   QMAX_Errado,  - K1_Errado,    K2_Errado);            // Teste ok   
ist::Sips     f71(   QMAX_Errado,            0,    K2_Errado);            // Teste ok   

ist::Sips     f8 (   QMAX_Errado,    K1_Errado,  - K2_Errado);            // Teste ok   
ist::Sips     f81(   QMAX_Errado,            K1_Errado,    0);            // Teste ok    
