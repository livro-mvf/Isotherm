//==============================================================================
// Name        : TesteRadkePrausnitsI.cpp
// Authors     : Lara Botelho Brum
//               Luan Rodrigues Soares de Souza
//               Joao Flavio Vieira de Vasconcellos
// Version     : 1.0
// Description : Programa para testar a classe com as 
//               equações da isoterma de Radke - Prausnits I.
//
...
//==============================================================================
    ...
    PrintLine(std::cout);    
    std::cout << "Teste da classe Radke - Prausnits I\n";
    PrintLine(std::cout);    
...
ist::RadkePrausnitsI     f1;                 //Construtora default    
    assert (f1.K1() == 0); 
    assert (f1.K2() == 0); 
    assert (f1.Qmax() == 0); 
...
ist::RadkePrausnitsI     f2( QMAX, K1, K2 );     // Construtora com 3 parâmetros; 
    assert (f2.K1() == K1); 
    assert (f2.K2() == K2); 
    assert (f2.Qmax() == QMAX);
...
ist::RadkePrausnitsI     f3(f2);             // Construtoa de cópia
    assert (f2.K1() == f3.K1()); 
    assert (f2.K2() == f3.K2()); 
...
ist::RadkePrausnitsI     f4 = ist::RadkePrausnitsI(QMAX1, K11, K22);
    assert (f4.K1() == K11); 
    assert (f4.K2() == K22); 
...
const ist::RadkePrausnitsI           var_RPI(QMAX_QE, K1_QE, K2_QE);
const Real                      QEANALITICO(0.845817508);
const Real                      QeNumerico(var_RPI.Qe(CE));
...
ist::RadkePrausnitsI     f6 ( - QMAX_Errado,    K1_Errado,    K2_Errado);            // Teste ok   

ist::RadkePrausnitsI     f7 (   QMAX_Errado,  - K1_Errado,    K2_Errado);            // Teste ok   

ist::RadkePrausnitsI     f8 (   QMAX_Errado,    K1_Errado,  - K2_Errado);            // Teste ok   

    std::cout   << "Imprimindo Qe com C errada = " 
                << f1.Qe( - CE_Errado ) 
                << std::endl;                                // Teste ok
    
ist::RadkePrausnitsI     f9;    
